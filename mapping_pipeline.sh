...
##########################################
# Commands 
# The dir just for the absolutely paths,check it carefully !!!  when you qsub your jobs it need for 24 threads
##########################################

SRA='WHF4-1Hi-C'    # The sample id
LABEL='Mi '         #
BWA='bwa'            #where is bwa
SAMTOOLS='samtools'  #where is samtools
IN_DIR='/public/home/dxbo/msq/sample/egg_Rep2/3'  #The idr of fastq files
REF='/public/home/dxbo/msq/contig/Mi_Wuhan.fasta'
FAIDX='$REF.fai'
RAW_DIR='/public/home/dxbo/msq/sample/egg_Rep2/3/rawdata_bam'
FILT_DIR='/public/home/dxbo/msq/sample/egg_Rep2/3/filter_bam'
FILTER='/public/home/dxbo/msq/tools/mapping_pipeline-master/filter_five_end.pl'
COMBINER='/public/home/dxbo/msq/tools/mapping_pipeline-master/two_read_bam_combiner.pl'
STATS='/public/home/dxbo/msq/tools/mapping_pipeline-master/get_stats.pl'
PICARD='/public/msq/tools/picard-tools-1.124/picard.jar'
TMP_DIR='/public/msq/WHF4-1/egg/sample1/temp/'
PAIR_DIR='/public/home/dxbo/msq/sample/egg_Rep2/3/paired_bam'
REP_DIR='/public/home/dxbo/msq/sample/egg_Rep2/3/Duplicate_bam'
REP_LABEL=$LABEL\_rep1
MERGE_DIR='/public/msq/WHF4-1/egg/sample1/'
MAPQ_FILTER=10
...
$BWA mem -t 24 -B 8 $REF $IN_DIR/$SRA\_R1.fastq | $SAMTOOLS view -Sb - > $RAW_DIR/$SRA\_1.bam

echo "### Step 1.B: FASTQ to BAM (2nd)"
$BWA mem -t 24 -B 8 $REF $IN_DIR/$SRA\_R2.fastq | $SAMTOOLS view -Sb - > $RAW_DIR/$SRA\_2.bam

echo "### Step 2.A: Filter 5' end (1st)"
$SAMTOOLS view -h -@ 24 $RAW_DIR/$SRA\_1.bam | perl $FILTER | $SAMTOOLS view -Sb -@ 24 - > $FILT_DIR/$SRA\_1.bam

echo "### Step 2.B: Filter 5' end (2nd)"
$SAMTOOLS view -h -@ 24 $RAW_DIR/$SRA\_2.bam | perl $FILTER | $SAMTOOLS view -Sb -@ 24 - > $FILT_DIR/$SRA\_2.bam

echo "### Step 3A: Pair reads & mapping quality filter"
perl $COMBINER $FILT_DIR/$SRA\_1.bam $FILT_DIR/$SRA\_2.bam $SAMTOOLS $MAPQ_FILTER | $SAMTOOLS view -bS -@ 24 -t $FAIDX - | $SAMTOOLS sort -o $TMP_DIR/$SRA.bam -
